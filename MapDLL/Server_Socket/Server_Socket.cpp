...
int __cdecl main()
{
    //define the default port to use.
    short port = 88888; // 39890;
    //define socket and initialize it to invalid socket
    ...
    // print a msg to inform that its ready for the messages from the controller
    printf("waiting for messages from controller\n");
    //receive until the controller shuts down the connection
    do {
        //receive a msg
       err = recvfrom(sock, recvbuf, recvsize, 0, (struct sockaddr*)&senderaddr, &senderaddrsize);
       //if a msg is recieved continue
        if (err > 0)
        {
            //copy the msg the variable message
            string message = "";
            for (int i = 0; i < recvsize; i++)
            {
                message = message + recvbuf[i];
            }
            //print the number of bytes recieved
            printf("Bytes received: %d\n", err);
            //check if the message is the one the server is expecting from the controller
            if (message == "Ready to process files. Start MAP.")
            {
                printf("receive the message\n");
                processAllFiles();
                printf("processed all files\n");
            }
            //prepare a msg to send to the controller
            message = "Processed files. Ready for the next message.";
            for (int i = 0; i < message.length(); i++)
            {
                recvbuf[i] = message[i];
            }
            recvbuf[message.length()] = '\n';
            //send  the new mesg back to the contoller 
            iSendResult = send(sock, recvbuf, err, 0);
            if (err == SOCKET_ERROR)
            {
                //if the sending was unsuccessful,print a msg and stop
                printf("Send failed with error: %d\n", WSAGetLastError());
                closesocket(sock);
                WSACleanup();
                return 1;
            }
            //if the sending was successful,print the number of bytes sent
            printf("Bytes sent: %d\n", iSendResult);
        }
       // if there is an error in receiving msgs, print the msg.
        else if (err == 0)
        {
        ...
        {
            //if there was an error in receiving the msg print the msg and stop
            printf("recv failed with error: %d\n", WSAGetLastError());
            closesocket(sock);
            WSACleanup();
            return 1;
        }
    } while (1 == 1); //heart beat(continue the process)
}
...
void processAllFiles()
{
    ...
    hDLL = LoadLibraryEx((LPCSTR)libName, NULL, NULL);   // Handle to DLL
    ...
    if (hDLL != NULL) {
        ...
        else
        {
            ...
            printf("Did not load map function correctly.\n");
        }
